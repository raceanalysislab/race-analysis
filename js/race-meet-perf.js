window.BOAT_CORE_MEET_PERF = (() => {
  ...
  function renderSlot(slot) {
    ...
    const isF = rank === "F";
    const botStyle = isF
      ? ' style="color:#d83939;font-weight:800;"'
      : "";

    return `
      <div class="meetPerfSlot">
        <div class="meetPerfSlotTop w${esc(waku || "")}">${esc(course || "")}</div>
        <div class="meetPerfSlotMid">${esc(st)}</div>
        <div class="meetPerfSlotBot"${botStyle}>${esc(rank)}</div>
      </div>
    `;
  }
  ...
  function normalizeDays(rawDays, fallbackWaku = "") {
    ...
    for (let dayIndex = 0; dayIndex < DAY_COUNT; dayIndex += 1) {
      ...
      for (let slotIndex = 0; slotIndex < SLOTS_PER_DAY; slotIndex += 1) {
        ...
        days[dayIndex][slotIndex] = srcSlot && typeof srcSlot === "object"
          ? {
              waku: srcSlot.waku ?? fallbackWaku ?? "",
    ...
    $meetPerfTable.innerHTML = `
      <div class="meetPerfRows">
        ${boats.map((boat) => {
          ...
          const days = normalizeDays(perfObj?.days, boat?.waku);
