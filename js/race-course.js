(() => {
  ...
  const formatRecentRank = (rec) => {
    return normalizeRecentRank(rec);
  };
  ...
  const renderWakuTrendRows = (
    ...
    return boats.map((boat) => {
      ...
      const recentCells = Array.from({ length: 10 }, (_, idx) => {
        ...
        const badgeWaku = rec?.waku ?? rec?.boat ?? waku;
        const course = rec?.course ?? rec?.boat ?? "—";
        const st = formatRecentSt(rec);
        const rank = formatRecentRank(rec);
        const rawResult = getRecentRawResult(rec);
        const isF = rawResult === "F";
        const rankColor = isF ? "#d83939" : "#0f172a";
        const orderNo = idx + 1;

        return `
          <div style="flex:0 0 37px;min-width:37px;height:100%;display:flex;flex-direction:column;border-right:1px solid #d7dde5;background:#fff;margin-bottom:-1px;">
            <div style="height:16px;display:flex;align-items:center;justify-content:center;border-bottom:1px solid #e8edf3;font-size:10px;font-weight:800;color:#64748b;background:#f8fafc;">
              ${orderNo}
            </div>
            <div style="height:18px;display:flex;align-items:center;justify-content:center;border-bottom:1px solid #e8edf3;">
              <div style="width:22px;height:16px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;${getRecentWakuBadgeStyle(badgeWaku)}">
                ${esc(course)}
              </div>
            </div>
            <div style="height:18px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#334155;border-bottom:1px solid #e8edf3;">
              ${esc(st)}
            </div>
            <div style="height:18px;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;line-height:1;color:${rankColor};padding-top:1px;border-bottom:0;">
              ${esc(rank)}
            </div>
          </div>
        `;
      ...
      return `
        <div style="display:grid;grid-template-columns:42px 112px minmax(0,1fr) 72px;border-bottom:1px solid #d7dde5;background:#fff;min-height:79px;">
          ...
          <div style="padding:7px 6px;border-right:1px solid #d7dde5;display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;min-width:0;${isFemale ? "background:#ffe4ec;" : ""}">
            <div style="font-size:10px;line-height:1.1;color:#6b7280;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;">
              ${esc(formatDash(boat?.regno))} / ${esc(formatDash(boat?.branch))} / ${esc(formatDash(boat?.age))}歳
            </div>
            <div style="display:flex;align-items:center;justify-content:center;gap:3px;width:100%;min-width:0;margin-top:3px;">
              ${isFemale ? '<span style="font-size:12px;line-height:1;color:#e2558f;font-weight:700;">♡</span>' : ""}
              <div style="font-size:14px;line-height:1.05;font-weight:800;color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;">
          ...
          <div style="display:grid;grid-template-rows:15px 23px 15px 23px;background:#fff;">
            ...
            <div style="display:flex;align-items:center;justify-content:center;border-bottom:1px solid #d7dde5;font-size:15px;font-weight:800;color:#1d4ed8;line-height:1;">
              ${esc(getWakuRecentAvgStText(boat, avgStField))}
            </div>
            <div style="display:flex;align-items:center;justify-content:center;border-bottom:1px solid #e8edf3;font-size:10px;font-weight:700;color:#64748b;line-height:1;background:#f8fafc;">
              勝率
            </div>
            <div style="display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:800;color:#475569;line-height:1;">
